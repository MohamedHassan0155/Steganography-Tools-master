...
def embed(frame, secret_message, key):
    # data=input("\nEnter the data to be Encoded in Video :") 
    data=encryption(secret_message, key)
...
def encode_vid_data(video_path, num, secret_message, encrypt_key):
    ...
    while(vidcap.isOpened()):
        ...
        if ret == False:
            break
        if frame_number == num:    
            change_frame_with = embed(frame, secret_message, encrypt_key)
...
def decode_vid_data(frame_, video_path, num, dencrypt_key):
    cap = cv2.VideoCapture(video_path)
    frame_ = np.load(frame_)
    print("****************************load********************************")
    print(frame_)
    max_frame=0;
    while(cap.isOpened()):
    ...
    while(vidcap.isOpened()):
        ...
        if ret == False:
            break
        if frame_number == num:
            msg = extract(frame_, dencrypt_key)
            return msg
